...
class Decoder(nn.Module):
    ...
    def forward_rl(self, fc_feats, sample_max=1,
                   decoding_constraint=0, max_seq_length=20):
        batch_size = fc_feats.size(0)
        fc_feats = self.fc_embed(fc_feats).unsqueeze(1)  # bz*1*emb_dim
        _, state = self.rnn(fc_feats)

        seq = fc_feats.new_zeros((batch_size, max_seq_length), dtype=torch.long)
        seq_logprobs = fc_feats.new_zeros((batch_size, max_seq_length))
        it = fc_feats.new_zeros(batch_size, dtype=torch.long).fill_(self.sos_id)  # first input <SOS>
        unfinished = it.new_ones(batch_size, dtype=torch.bool)
